...
#DB连接
def db():
	conn = pymysql.connect(host="10.21.200.48",user="opinion",passwd="vDGM0lspmy=", db="opinion", charset="utf8", cursorclass = pymysql.cursors.DictCursor)
	conn.ping(True)
	return conn
...
def main(d, uname):
	...
	while True:
		...
		sql_upload = "update xiaohongshu_note_usr2 set flag = 10,upload = upload + 1 where user_id = '" + str(user_id) + "';"
		...
		if d(resourceId="com.xingin.xhs:id/fh").exists():
			...
			if d(resourceId="com.xingin.xhs:id/c_c").exists():
				...
				if d(resourceId="com.xingin.xhs:id/a0d", text="还没有关注任何小红薯哦").exists():
					sql_upload = "update xiaohongshu_note_usr2 set flag = 20 where user_id = '" + str(user_id) + "';"
					cursor.execute(sql_upload)
					dbn.commit()
					break
				...
				if d(resourceId="com.xingin.xhs:id/c97", text="话题和专辑").exists():
					...
					if d(className="android.widget.TextView", text="话题").exists():
						if d(resourceId="com.xingin.xhs:id/c_c", text="还没有关注任何话题哦").exists():
							d.press("back")
							break
